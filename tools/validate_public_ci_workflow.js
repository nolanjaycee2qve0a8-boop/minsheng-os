...
function validatePublicCiWorkflow(file = workflow) {
  ...
  for (const [index, line] of lines.entries()) {
    ...
    if (!/^\s*(?:-\s+)?[A-Za-z][A-Za-z0-9_\- ]*:\s*(?:.*)?$/.test(line) && !/^\s*-\s+uses:\s+[^\s]+(?:\s+#.+)?$/.test(line) && !/^\s*-\s+name:\s+.+$/.test(line) && !/^\s*\}\}\s*$/.test(line)) {
      throw new Error(`YAML_STRUCTURE_INVALID:${index + 1}`);
    }
  }
  const required = [
    'name: Public CI', 'push:', 'pull_request:', 'branches: [main]', 'permissions:', 'contents: read',
    'cancel-in-progress: true', 'timeout-minutes: 20', 'actions/checkout@11bd71901bbe5b1630ceea73d27597364c9af683 # v4.2.2', 'fetch-depth: 0',
    'actions/setup-node@49933ea5288caeca8642d1e84afbd3f7d6820020 # v4.4.0', "node-version: '22'",
